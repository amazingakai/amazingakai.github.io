+++
title = 'My First Few Months Contributing to OpenTelemetry Go Compile Instrumentation'
...
I was building a Discord minigame as a side project. It grew bigger than I
expected, and at some point I needed a way to actually see what was going
on inside it when things broke. I didn't know the word for this yet. I
just knew I needed it.

That's how I found OpenTelemetry. While looking around the project, I came
across [OpenTelemetry Go Compile Time Instrumentation](https://github.com/open-telemetry/opentelemetry-go-compile-instrumentation),
a tool that instruments Go programs at compile time instead of requiring
developers to hand-write tracing code. I liked the idea a lot, so I
started contributing.

Around the same time, I came across LFX Mentorship. I proposed an idea,
and it got accepted, which I was very happy about. During the
application period, the project got a lot of traction, and alongside
contributing code, I also started doing code reviews. That turned out to
be one of the more useful things I did during that period, reviewing
other people's changes taught me things I wouldn't have picked up just by
writing my own code. Eventually, I got selected.
...
technically and in terms of how contributing to a real project actually works. 
I've written about that in more detail separately ([link](/posts/gsoc-2025-expanding-oss-fuzz-integration-across-kde-libraries)), 
and it's worth mentioning here because it's a big part of why I felt ready
to jump into OTel later.

## Why Mentorship again?

Two reasons, really.

The first was that I wanted to get better at communicating. LFX comes with
weekly SIG meetings and regular meetings with my mentors, [Kemal Akkoyun](https://github.com/kakkoyun) and
[Dario Castañé](https://github.com/darccio). Having to regularly explain what I've
been working on to people who know the project well has been good practice,
especially since it isn't something that comes naturally to me.
...
still in the middle of the mentorship, but so far it's been exactly what I
was hoping for.
...
![OpenTelemetry Go Compile Instrumentation Import-Driven Instrumentation Flow](/images/otelc-import-driven-instrumentation-flow-light.png)
![OpenTelemetry Go Compile Instrumentation Import-Driven Instrumentation Flow](/images/otelc-import-driven-instrumentation-flow-dark.png)
...
![OpenTelemetry Go Compile Instrumentation initInstrumentation Deadlock Sequence](/images/otelc-initinstrumentation-deadlock-sequence-light.png)
![OpenTelemetry Go Compile Instrumentation initInstrumentation Deadlock Sequence](/images/otelc-initinstrumentation-deadlock-sequence-dark.png)
...
![OpenTelemetry Go Compile Instrumentation Auto-Pinning Before/After](/images/otelc-autopinning-before-after-light.png)
![OpenTelemetry Go Compile Instrumentation Auto-Pinning Before/After](/images/otelc-autopinning-before-after-dark.png)

This also gives us a cleaner path toward
[registry-backed integration search](#whats-next). If otelc eventually
discovers compatible instrumentations from the OpenTelemetry registry,
the same pinning step can resolve those integrations and their dependencies
before rule matching, rather than requiring the rest of the build pipeline
to care where an instrumentation came from. I talk more about that idea in
the registry-backed integration section below.
...
2. **Registry-backed integration search.** Instead of only looking at the
   ...
   through the OpenTelemetry registry, which would let third-party
   instrumentations get picked up automatically once published there.
   The proposal is currently in the RFC stage and open for review.
...
Coming into this mentorship, I wanted to get better at Go, learn more
about observability, and become more comfortable talking about my work.
It's still ongoing, but I already feel much more comfortable discussing design 
ideas in reviews and speaking up during meetings than I did at the start. 
I'll write more once the mentorship wraps up.
